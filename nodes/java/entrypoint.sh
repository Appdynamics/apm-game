...
then
	...
	then
		java -javaagent:/opt/appdynamics/javaagent.jar -Djava.security.egd=file:/dev/urandom -Dappdynamics.socket.collection.bci.enable=true -Dappdynamics.analytics.agent.url=http://machine-agent:9090/v2/sinks/bt  -jar "javanode-1.0-SNAPSHOT.jar" 80
	else
		...
		java -javaagent:/opt/appdynamics/javaagent.jar -Djava.security.egd=file:/dev/urandom -Dappdynamics.socket.collection.bci.enable=true -Dappdynamics.dockerMonitoring=true -jar "javanode-1.0-SNAPSHOT.jar" 80
	fi
else
